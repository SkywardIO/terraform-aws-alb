...
module "access_logs" {
  ...
  version                            = "0.11.3"
  enabled                            = module.this.enabled && var.access_logs_enabled && var.access_logs_bucket_id == "" ? true : false
...
locals {
  bucket_id = var.access_logs_bucket_id != "" ? var.access_logs_bucket_id : module.access_logs.bucket_id
}
